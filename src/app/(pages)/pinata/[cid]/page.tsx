...
const CID = ({ fileData }: any) => {
  ...
  const download = async () => {
    const res = await fetch(
      `${GATEWAY_URL}/ipfs/${fileData.ipfs_pin_hash}?download=true`
    );
            ...
            <div className="h-full max-w-screen-xl">
              <div className="m-auto flex h-full w-full items-center justify-center">
                <div className="m-auto w-3/4 text-center">
                  <h1>Download file</h1>
                  <p className="mt-2">
                    Please make sure you trust the source of this link. If you
                    dot not know who sent you the link and are unsure what will
                    be downloaded, do not click the download button.
                  </p>
                  <a
                    className="hidden"
                    href={href}
                  ...
                  />
                  <div className="mt-8 flex flex-col items-center justify-center rounded-lg bg-light p-2 text-center align-center h-64 w-3/4 m-auto l bg-secondary px-4 py-2 text-light transition-all duration-300 ease-in-out hover:bg-accent hover:text-light">
